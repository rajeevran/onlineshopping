...
export default async function handler(req, res) {
  ...
  if (req.method === "POST") {
    ...
    try{const created=await RecommendedProduct.create(data);return res.status(201).json(created);}catch(err){return res.status(400).json({message:err.message||"Unable to create record"});}
