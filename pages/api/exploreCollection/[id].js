...
export default async function handler(req, res) {
  ...
  await connectToDatabase();
    ...
    if (req.method === "GET") {
    const query = ExploreCollection.findById(id)
    ...
    if (req.method === "PUT") {
      ...
      const updated = await ExploreCollection.findByIdAndUpdate(id, data, { new: true, runValidators: true });
      if (!updated) return res.status(404).json({ message: "Item not found" });

      const item = await ExploreCollection.findById(updated._id)
        .populate("userId", "name email")
        .populate("productId", "name price images");
      return res.status(200).json(item);
    }
    ...
    if (req.method === "DELETE") {
      const item = await ExploreCollection.findByIdAndDelete(id);
      if (!item) return res.status(404).json({ message: "Item not found" });
    ...
    console.error("[exploreCollection API]", error);
