...
export default async function handler(req, res) {
  try {
    ...
    if (req.method === "GET") {
    const query = HomeProduct.findById(id)
    ...
    if (req.method === "PUT") {
      ...
      const updated = await HomeProduct.findByIdAndUpdate(id, data, { new: true, runValidators: true });
      if (!updated) return res.status(404).json({ message: "Item not found" });

      const item = await HomeProduct.findById(updated._id)
        .populate("productId", "name price images");
      return res.status(200).json(item);
    }

    if (req.method === "DELETE") {
      const item = await HomeProduct.findByIdAndDelete(id);
      if (!item) return res.status(404).json({ message: "Item not found" });
    ...
    console.error("[homeproducts API]", error);
