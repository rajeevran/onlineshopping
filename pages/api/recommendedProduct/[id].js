...
const updated=await RecommendedProduct.findByIdAndUpdate(id,payload,{new:true,runValidators:true});if(!updated)return res.status(404).json({message:"Record not found"});return res.status(200).json(updated);}catch(err){return res.status(400).json({message:err.message||"Unable to update record"});}
  }

  if (req.method === "DELETE") {
    await RecommendedProduct.findByIdAndDelete(id);
